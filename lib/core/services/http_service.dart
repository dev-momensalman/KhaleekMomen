...
class HttpService {
  ...
  Future<dynamic> get(String path, {Map<String, String>? headers}) async {
    ...
    try {
      ...
      throw NetworkException(message: 'Connection timed out. Please check your internet connection.');
    } on http.ClientException catch (e) {
      developer.log('GET client exception: ${e.message}', name: 'HttpService');
      throw NetworkException(message: 'Failed to connect to the server. Please check your connection.');
    } catch (e) {
  ...
  dynamic _processResponse(http.Response response) {
    ...
    developer.log('Response status: $statusCode for ${response.request?.url}', name: 'HttpService');

    if (statusCode >= 200 && statusCode < 300) {
      if (response.body.isEmpty) return null;
      return jsonDecode(response.body);
    } else if (statusCode == 401 || statusCode == 403) {
      throw NetworkException(message: 'Unauthorized access.', statusCode: statusCode);
    } else if (statusCode >= 400 && statusCode < 500) {
      throw NetworkException(message: 'Client error occurred.', statusCode: statusCode);
    } else {
      throw NetworkException(message: 'Server error. Please try again later.', statusCode: statusCode);
    }
  }
}
