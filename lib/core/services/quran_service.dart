...
class QuranService {
  ...
  final List<Surah> _surahMetadataList = [
    ...
    Surah(number: 38, name: 'ص', englishName: 'Sad', englishNameTranslation: 'The Letter Sad', numberOfAyahs: 88, revelationType: 'Meccan'),
    ...
    Surah(number: 50, name: 'ق', englishName: 'Qaf', englishNameTranslation: 'The Letter Qaf', numberOfAyahs: 45, revelationType: 'Meccan'),
    ...
    Surah(number: 62, name: 'الجمعة', englishName: 'Al-Jum\'ah', englishNameTranslation: 'The Congregation', numberOfAyahs: 11, revelationType: 'Medinan'),
    ...
    Surah(number: 92, name: 'الليل', englishName: 'Al-Lail', englishNameTranslation: 'The Night', numberOfAyahs: 21, revelationType: 'Meccan'),
    ...
    Surah(number: 95, name: 'التي��', englishName: 'At-Tin', englishNameTranslation: 'The Fig', numberOfAyahs: 8, revelationType: 'Meccan'),
    ...
    Surah(number: 102, name: 'التكاثر', englishName: 'At-Takathur', englishNameTranslation: 'Competition', numberOfAyahs: 8, revelationType: 'Meccan'),
    ...
    Surah(number: 106, name: 'قريش', englishName: 'Quraish', englishNameTranslation: 'Quraish', numberOfAyahs: 4, revelationType: 'Meccan'),
    Surah(number: 107, name: 'الماعون', englishName: 'Al-Ma\'un', englishNameTranslation: 'Almsgiving', numberOfAyahs: 7, revelationType: 'Meccan'),
    Surah(number: 108, name: 'الكوثر', englishName: 'Al-Kauthar', englishNameTranslation: 'Abundance', numberOfAyahs: 3, revelationType: 'Meccan'),
    ...
    Surah(number: 110, name: 'النصر', englishName: 'An-Nasr', englishNameTranslation: 'Divine Support', numberOfAyahs: 3, revelationType: 'Medinan'),
  ...
  Future<List<Reciter>> getReciters() async {
    ...
    try {
      ...
      if (response != null && response['reciters'] != null) {
        ...
        final apiReciters = recitersJson.map((item) {
          ...
          if (item['moshaf'] != null && item['moshaf'] is List) {
            ...
            if (moshafs.isNotEmpty) {
              ...
              styleName = moshafs[0]['name']?.toString() ?? 'Hafs A\'n Assem';
            }
          }
        ...
        }).where((r) => r.name.isNotEmpty && r.server.isNotEmpty).toList();

        if (apiReciters.isNotEmpty) {
          developer.log('Successfully fetched ${apiReciters.length} reciters from API.', name: 'QuranService');
          return apiReciters;
        }
      }
      throw Exception('Empty or invalid reciters response.');
    } catch (e) {
      developer.log('Failed to fetch reciters online: $e. Returning fallback default reciters.', name: 'QuranService');
  ...
  List<Surah> getSurahs(Reciter reciter) {
    developer.log('Getting surahs list for reciter: ${reciter.name}', name: 'QuranService');
    ...
    return _surahMetadataList.map((surah) {
      ...
      final baseUrl = reciter.server.endsWith('/') ? reciter.server : '${reciter.server}/';
  ...
  Future<List<Ayah>> getSurahVerses(int surahNumber) async {
    developer.log('Fetching verses for surah: $surahNumber', name: 'QuranService');
    try {
      ...
      if (_sampleVerses.containsKey(surahNumber)) {
        developer.log('Returning cached sample verses for surah $surahNumber', name: 'QuranService');
  ...
  static final Map<int, List<Ayah>> _sampleVerses = {
    1: [
      Ayah(number: 1, text: 'بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ', translation: 'In the name of Allah, the Entirely Merciful, the Especially Merciful.'),
      Ayah(number: 2, text: 'الْحَمْدُ لِلَّهِ رَبِّ الْعَالَمِينَ', translation: '[All] praise is [due] to Allah, Lord of the worlds -'),
      Ayah(number: 3, text: 'الرَّحْمَٰنِ الرَّحِيمِ', translation: 'The Entirely Merciful, the Especially Merciful,'),
      Ayah(number: 4, text: 'مَالِكِ يَوْمِ الدِّينِ', translation: 'Sovereign of the Day of Recompense.'),
      Ayah(number: 5, text: 'إِيَّاكَ نَعْبُدُ وَإِيَّاكَ نَسْتَعِينُ', translation: 'It is You we worship and You we ask for help.'),
      Ayah(number: 6, text: 'اهْدِنَا الصِّرَاطَ الْمُسْتَقِيمَ', translation: 'Guide us to the straight path -'),
      Ayah(number: 7, text: 'صِرَاطَ الَّذِينَ أَنْعَمْتَ عَلَيْهِمْ غَيْرِ الْمَغْضُوبِ عَلَيْهِمْ وَلَا الضَّالِّينَ', translation: 'The path of those upon whom You have bestowed favor, not of those who have evoked [Your] anger or of those who are astray.'),
    ],
    112: [
      Ayah(number: 1, text: 'قُلْ هُوَ اللَّهُ أَحَدٌ', translation: 'Say, "He is Allah, [who is] One,'),
      Ayah(number: 2, text: 'اللَّهُ الصَّمَدُ', translation: 'Allah, the Eternal Refuge.'),
      Ayah(number: 3, text: 'لَمْ يَلِدْ وَلَمْ يُولَدْ', translation: 'He neither begets nor is born,'),
      Ayah(number: 4, text: 'وَلَمْ يَكُن لَّهُ كُفُوًا أَحَدٌ', translation: 'And there is none co-equal or comparable unto Him."'),
    ],
  };
  ...
  Future<List<Ayah>> getAyahTafsir(int surahNumber, int ayahNumber) async {
    developer.log('Fetching tafsir for $surahNumber:$ayahNumber', name: 'QuranService');
    try {
      final path = '/ayah/$surahNumber:$ayahNumber/editions/quran-uthmanic,en.asad,ar.muyassar';
      final response = await _textHttpService.get(path);
